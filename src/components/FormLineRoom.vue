...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$http.post("http://182.92.122.205:8080/", argu_post).then(res => {
        ...
        this.$store.commit("capAndadd", {
          capacity: this.room_capcity,
          address: this.room_address
        });
      });
    },
    open3() {
  ...
  computed: {
    ...mapState({
      ...
      gitcapacity: state => state.labroom.capacity,
      gitaddress: state => state.labroom.address
    })
  },
  beforeMount: function() {
    ...
    this.$http
      .post("http://182.92.122.205:8080/", "gettable=address")
      .then(res => {
        var str = res.data;
        str = str.replace(/'/g, '"');
        var resobj = JSON.parse(str);
